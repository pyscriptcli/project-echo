...
- `pages/` — `0_admin.py` (user mgmt), `1_minutes_of_the_meeting.py` (MOM editor/exports), `2_meeting_details.py` (records + PDF/DOCX exports), `3_echo_ai.py` (chat), `4_tasks.py` (task board), `6_notebook.py`, `8_documents.py` (template-based PPTX/DOCX generator — **local-only, never writes to the DB**; DOCX-first with an optional **Download PDF** that auto-detects `docx2pdf`/Word → LibreOffice `soffice`). Template files for the Documents page live in `documents_template/` (committed, protected from app deletion); app-uploaded templates and their configs are saved to `stored_templates/` (gitignored). Templates are stored as files on GitHub, not in the DB. (Project Atlas and Project Node were removed from the app and repo.)
