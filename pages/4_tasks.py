...
# -------------------------------------------------------------------------
# Helper: Insert a task into Supabase
# -------------------------------------------------------------------------
def insert_task_from_row(meeting_id, discussion, action, due_date_str, pic):
    """Insert a new task into the 'tasks' table using a meeting action row."""
    supabase = get_supabase_client()
    if not supabase:
        st.error("Supabase client not initialized.")
        return False

    # Parse due date if possible
    parsed_date = None
    if due_date_str:
        try:
            ...
            for fmt in ("%B %d, %Y", "%m/%d/%Y"):
                try:
                    parsed_date = datetime.strptime(due_date_str, fmt).date()
                    break
                except:
                    pass

    payload = {
        "title": action.strip() if action.strip() else discussion.strip()[:120],
        "description": discussion.strip(),
        "assignee": pic.strip() if pic else None,
    ...
    with tab_editor:
        with st.container(border=True):
            ...
            for idx, row in enumerate(rows):
                with st.container(border=True):
                    # Updated column layout: add a sixth column for "Add to Task"
                    c_disc, c_act, c_date, c_pic, c_del, c_add = st.columns([3.2, 3.2, 1.8, 1.8, 0.6, 0.8])
                    ...
                    # New column: Add to Task button
                    with c_add:
                        st.write("<div style='height: 38px;'></div>", unsafe_allow_html=True)
                        task_added_flag = f"task_added_{m_id}_{idx}"
                        if task_added_flag in st.session_state and st.session_state[task_added_flag]:
                            st.button("✅ Added", key=f"added_{m_id}_{idx}", disabled=True, help="Already added to tasks")
                        else:
                            if st.button("📌", key=f"addtask_{m_id}_{idx}", help="Add to Tasks"):
                                success = insert_task_from_row(
                                    meeting_id=m_id,
                                    discussion=st.session_state[f"dp_{m_id}_{idx}"],
                                    action=st.session_state[f"ap_{m_id}_{idx}"],
                                    due_date_str=st.session_state[f"date_{m_id}_{idx}"],
                                    pic=st.session_state[f"pic_{m_id}_{idx}"]
                                )
                                if success:
                                    st.session_state[task_added_flag] = True
