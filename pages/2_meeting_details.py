...
def export_to_word_template_2(df, meeting_details, other_discussions):
    ...
    def add_clean_para(text="", bold=False, italic=False, font_size=11, space_before=0, space_after=4, color_rgb=(0, 0, 0)):
        ...
        if text:
            ...
            run.bold = bold
            run.italic = italic
            run.font.name = docfonts.WORD_BODY
    ...
    for i, (k, v) in enumerate(details_map):
        ...
        for c in [c0, c1]:
            ...
            p.paragraph_format.space_after = Pt(2)
            if p.runs:
                p.runs[0].font.name = docfonts.WORD_BODY
    ...
    for att in all_atts:
        if att.strip():
            ...
            r.font.name = docfonts.WORD_BODY
            r.font.size = Pt(10)
    ...
    for i, row in df.iterrows():
        ...
        if dp:
            ...
            r_num.font.name = docfonts.WORD_BODY
            r_num.font.size = Pt(10)
            r_txt = p_dp.add_run(dp)
            r_txt.font.name = docfonts.WORD_BODY
